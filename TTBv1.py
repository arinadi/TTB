# @title 🚀 Run Transcription Bot (Telegram Version - Final with Custom Cleanup)
# ------------------------------------------------------------------------------
...
try:
    ...
    if not all([TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID]):
        raise ValueError("Core secrets (TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID) must be set in Colab.")
    TELEGRAM_CHAT_ID = int(TELEGRAM_CHAT_ID)
    if not GEMINI_API_KEY:
        print("⚠️ WARNING: GEMINI_API_KEY not set. Summarization features will be disabled.")
    print("✅ Secrets loaded successfully.")
except Exception as e:
    print(f"❌ ERROR: Failed to load secrets: {e}", file=sys.stderr)
    sys.exit("Execution stopped due to missing secrets.")


# --- 1.2. Bot & Model Configuration ---
#@markdown ---
#@markdown ### 🤖 Model & Transcription Settings
model_size = 'large-v3' #@param ['large-v3', 'large-v2', 'medium', 'small', 'base', 'tiny']
use_fp16 = 'auto' #@param ['auto', 'True', 'False']
beam_size = 10 #@param {type:"integer"}
pause_threshold = 0.3 #@param {type:"number"}
MAX_AUDIO_DURATION_MINUTES = 90 #@param {type:"integer"}
#@markdown ---
#@markdown ### 🕒 Runtime Management
ENABLE_IDLE_MONITOR = True #@param {type:"boolean"}
IDLE_NOTIFY_MINUTES = 5 #@param {type:"integer"}
IDLE_WARNING_MINUTES = 8 #@param {type:"integer"}
IDLE_SHUTDOWN_MINUTES = 10 #@param {type:"integer"}
#@markdown ---
#@markdown ### 📁 File Handling
MAX_FILE_SIZE_MB = 20 #@param {type:"integer"}
#@markdown ---

# --- 1.3. Derived Constants ---
MAX_DURATION_IN_SECONDS = MAX_AUDIO_DURATION_MINUTES * 60
...
print("\n✅ Verifying pre-installed libraries...")
try:
    print(f"✅ PyTorch version {torch.__version__} is already installed.")
    if torch.cuda.is_available():
        print(f"✅ PyTorch is linked with CUDA {torch.version.cuda}")
except Exception as e:
    print(f"⚠️ Could not verify PyTorch. This may cause issues. Error: {e}")
...
!pip install -q ffmpeg-python python-telegram-bot --pre requests werkzeug google-generativeai tqdm more-itertools nest_asyncio
...
print("✅ Volatile storage paths created.")


# ------------------------------------------------------------------------------
# SECTION 3: AI AND HARDWARE INITIALIZATION
# ------------------------------------------------------------------------------
print("\n🤖 Initializing AI and hardware settings...")
device = "cuda" if torch.cuda.is_available() else "cpu"
if device == "cuda": print("✅ GPU (CUDA) detected.")
else: print("⚠️ GPU (CUDA) not detected. Using CPU, which will be much slower.")

fp16_enabled = str(use_fp16).lower() == 'true' or (str(use_fp16).lower() == 'auto' and device == 'cuda')
if fp16_enabled: print("✅ FP16 precision is enabled for faster performance on GPU.")

# Models will be loaded asynchronously after the bot starts.
model = None
gemini_client = None
...
# ------------------------------------------------------------------------------
# SECTION 4: CORE UTILITIES AND DATA CLASSES
# ------------------------------------------------------------------------------
print("\n🏛️ Defining core architecture and utilities...")
...
async def summarize_text(transcript: str) -> str:
    ...
    try:
        # <<< --- FIX DI SINI: Menjalankan kode sinkron dari dokumentasi di dalam lingkungan asinkron bot ---
...
class IdleMonitor:
    ...
    async def _monitor_loop(self):
        while True:
            ...
            if self.shutdown_imminent or not ENABLE_IDLE_MONITOR:
                continue
            ...
            if self.job_manager.is_idle():
                ...
                if idle_duration_minutes >= IDLE_SHUTDOWN_MINUTES:
                    self.shutdown_imminent = True
                    shutdown_msg = f"🔴 *AUTO SHUTDOWN:*\nBot has been idle for over *{IDLE_SHUTDOWN_MINUTES}* minutes."
                    await send_telegram_notification(self.app, shutdown_msg)
                    await perform_shutdown("Automatic Idle Shutdown")
                elif idle_duration_minutes >= IDLE_WARNING_MINUTES and not self.warnings_sent['warning']:
                    warn_msg = f"⚠️ *IDLE WARNING:*\nBot will shut down in approx. *{IDLE_SHUTDOWN_MINUTES - int(idle_duration_minutes)}* minutes."
                    await send_telegram_notification(self.app, warn_msg)
                    self.warnings_sent['warning'] = True
                elif idle_duration_minutes >= IDLE_NOTIFY_MINUTES and not self.warnings_sent['notify']:
...
class JobManager:
    ...
    def complete_job(self, job_id: str):
        # Clears the currently processing job if it matches the completed job ID.
        if self.currently_processing and self.currently_processing.job_id == job_id:
            self.currently_processing = None
        # Removes the job from the central tracking registry.
        if job_id in self.job_registry:
            del self.job_registry[job_id]
        # FIX: Use 'job_id' directly, as the 'job' object is not in scope here.
        print(f"[JOB:{job_id}] Job completed and removed from registry.")
        # Reset the idle timer as an action has just completed.
        self.idle_monitor.reset()
...
# SECTION 5: TELEGRAM UI AND COMMAND HANDLERS
# ------------------------------------------------------------------------------
print("\n🎨 Defining Telegram commands and UI components...")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "refresh_status":
        text, reply_markup = await get_status_text_and_keyboard()
        try:
            # Prevents an error if the user clicks refresh on an unchanged message.
...
# ------------------------------------------------------------------------------
# SECTION 6: FILE HANDLING AND VALIDATION
# ------------------------------------------------------------------------------
...
class FilesHandler:
    ...
    def __init__(self, job_manager: JobManager, upload_folder: str):
        ...
        self.multipart_archives = {} # State: {base_name: {'files': [...], 'timer': handle, 'message': message}}
    ...
    async def _validate_and_queue_file(self, local_path: str, message: telegram.Message, filename_override: str = None):
        """Validates a single media file for duration and format, then queues it."""
        try:
            ...
            # For files extracted from ZIPs, create a mock message object for the job.
            job_message = message
    ...
    async def handle_files(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Entry point for handling attachments. Checks size and routes to the correct handler."""
        message = update.effective_message
        if not (message and message.effective_attachment): return

        attachment = message.effective_attachment

        # Pre-check: Reject files larger than the Telegram Bot API download limit.
        if attachment.file_size and attachment.file_size > MAX_FILE_SIZE_BYTES:
            ...
            await message.reply_text(
                ...
                f"*{MAX_FILE_SIZE_MB} MB*. Please send a smaller file.",
        ...
        # Route file based on its name and extension.
    ...
    async def _handle_multipart_chunk(self, message: telegram.Message, file_obj: telegram.File, match: re.Match):
        """Handles a single chunk of a multi-part archive (e.g., file.zip.001)."""
    ...
    async def _process_multipart_archive(self, base_name: str):
        """Combines and processes a multi-part archive after the timeout."""
        ...
        # Sort chunks numerically based on their extension (e.g., .001, .002).
    ...
    async def _extract_and_queue_zip(self, zip_path: str, zip_name: str, message: telegram.Message, status_message: telegram.Message = None):
        """Extracts a ZIP archive and queues its valid contents."""
        ...
        try:
            ...
            for root, _, files in os.walk(extract_dir):
                for filename in files:
                    ...
                    # Move to main upload folder to standardize pathing for jobs.
...
# ------------------------------------------------------------------------------
# SECTION 7: BOT WORKER AND LIFECYCLE MANAGEMENT
# ------------------------------------------------------------------------------
print("\n🤖 Initializing Telegram bot and background worker...")
...
async def initialize_models_background():
    ...
    try:
        ...
        model = await asyncio.to_thread(whisper.load_model, model_size, device=device)
        print(f"✅ [BG Task] Whisper model '{model_size}' loaded.")
        if GEMINI_API_KEY:
            print("⏳ [BG Task] Initializing Gemini client...")
            # <<< --- FIX DI SINI: Menggunakan genai.Client persis seperti dokumentasi ---
...
async def queue_processor():
    ...
    while not SHUTDOWN_IN_PROGRESS:
        ...
        try:
            ...
            summary_text = await summarize_text(transcript_text)
            ...
            # Clean up the original uploaded audio file to save space for the next job.
            if os.path.exists(job.local_filepath):
                os.remove(job.local_filepath)

            # Per user request, the generated transcript and summary files are NOT deleted.
            # They will be cleared automatically when the Colab runtime is unassigned.

            # Explicitly free up memory from the potentially large transcript text variable.
...
async def main():
    ...
    # Use a custom request object with longer timeouts for better network resilience.
    request = HTTPXRequest(read_timeout=60.0, connect_timeout=20.0)
    application = Application.builder().token(TELEGRAM_BOT_TOKEN).request(request).build()

    # Initialize core components and link them.
    idle_monitor = IdleMonitor(application, None) # JobManager is set after its own init.
    ...
    # MODIFIED: Wording updated as per user request for precision.
    startup_message = (
        f"🚀 *Bot is starting up...*\n\n"
        f"*Model:* `{model_size}` on `{device.upper()}`\n"
        f"*Idle Monitor:* `{'Enabled' if ENABLE_IDLE_MONITOR else 'Disabled'}`\n"
        f"*Bot Handle Limit:* `{MAX_FILE_SIZE_MB} MB` per file.\n\n"
        f"ℹ️ *Usage Tips:*\n"
        f"- All audio & video formats supported by FFmpeg are accepted.\n"
        f"- For files exceeding the limit, please use a multi-part ZIP archive (e.g., `file.zip.001`, `file.zip.002`)."
    )
...
if __name__ == "__main__":
    try:
        ...
        err_msg = f"❌ **CRITICAL ERROR:** The bot was stopped unexpectedly.\n`{e}`"
        print(err_msg, file=sys.stderr)
        # Attempt to send a final notification if the application object was initialized.
        if 'application' in globals() and application and application.initialized:
            loop = asyncio.get_event_loop()
            if not loop.is_closed():
                loop.run_until_complete(send_telegram_notification(application, err_msg))